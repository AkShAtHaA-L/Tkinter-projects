...
def generate():
    ...
    number_list = [str(i) for i in range(0, 9)]
    symbol_list = ['!','@','#','$','%','^','&','*','\\','/','?']
    password_list = random.choices(alphabet_list, k=5) + random.choices(number_list, k=4) + random.choices(symbol_list, k=2)
    random.shuffle(password_list)
    genpassword = "".join(password_list)
    password.insert(0, genpassword)
...
#Generate a password
generate_btn = Button(root, text = "Generate password", command = generate)
generate_btn.grid(row=8, column=0, ipadx=30)
